import { Link } from "@remix-run/react";
import { useState } from "react";

export default function AppNavBar() {
	const links = [
		{
			label: "Products",
			url: "#",
		},
		{
			label: "Solutions",
			url: "#",
		},
		{
			label: "Platform",
			url: "#",
		},
		{
			label: "Resources",
			url: "#",
		},
		{
			label: "Pricing",
			url: "#",
		},
	];

	const [openMenu, setOpenMenu] = useState(false);

	return (
		<section
			id="mainNavigationMenu"
			className="sticky z-50 -top-px md:shadow-sm md:border-b bg-canvas border-content/10"
		>
			<div className="relative flex items-center justify-between p-3 md:hidden">
				<Link href="/" className="pb-2 ml-1">
					<svg
						id="logoSvg"
						className="h-7"
						viewBox="0 0 116 34"
						fill="currentColor"
						xmlns="http://www.w3.org/2000/svg"
					>
						<path
							d="M45.6952 12.9354C49.0073 12.9354 51.6872 10.2295 51.6872 6.89741C51.6872 3.56533 49.0019 0.859375 45.6952 0.859375C42.3885 0.859375 39.7031 3.56533 39.7031 6.89741C39.7031 10.2295 42.3885 12.9354 45.6952 12.9354Z"
							fill="#F8B400"
						/>
						<path d="M11.3357 14.2969H0V33.1133H5.33827V18.3367H10.0011C11.1142 18.3367 12.0057 19.2351 12.0057 20.3567V33.1351H17.344V20.3567C17.344 16.9974 14.6424 14.2969 11.3303 14.2969H11.3357Z" />
						<path d="M70.0436 29.0717H66.5478C65.5158 28.99 64.7 28.1297 64.7 27.0517V18.3132H70.0382V14.2733H64.7V7.55469H59.3617V14.2733H57.0222L54.0234 18.3132H59.3617V27.068C59.3617 30.4056 62.0416 33.1061 65.3537 33.1061H70.0166V30.8684L70.0382 29.0662L70.0436 29.0717Z" />
						<path d="M55.7192 14.2969H49.7056L45.7181 19.6761L41.709 14.2969H35.6953L42.7031 23.6942L35.6953 33.1133H41.709L45.7181 27.7341L49.7056 33.1133H55.7192L48.7114 23.6942L55.7192 14.2969Z" />
						<path d="M92.7187 14.2969L88.0558 26.2749L83.3929 14.2969H78.0547L85.3975 33.1133H90.7196L98.0624 14.2969H92.7241H92.7187Z" />
						<path d="M76.7211 14.2969H71.3828V33.1133H76.7211V14.2969Z" />
						<path d="M76.7211 7.57812H71.3828V12.9574H76.7211V7.57812Z" />
						<path d="M28.0114 29.0718C26.3257 29.0718 24.8938 27.6725 24.2995 25.7125H37.1427C37.2832 25.07 37.3588 24.3949 37.3588 23.6926C37.3588 18.493 33.1714 14.2734 28.0114 14.2734C22.8515 14.2734 18.6641 18.493 18.6641 23.6926C18.6641 28.8921 22.8515 33.1117 28.0114 33.1117H34.3439L37.3426 29.0718H28.0114ZM28.0114 18.3351C29.6972 18.3351 31.129 19.7343 31.7234 21.6944H24.2995C24.8938 19.7126 26.3419 18.3351 28.0114 18.3351Z" />
						<path d="M109.11 14.2969H99.3846L97.8177 18.3367H107.781C108.894 18.3367 109.786 19.2351 109.786 20.3567V21.6961H101.784V21.7178C98.8065 21.8975 96.4453 24.3748 96.4453 27.4183C96.4453 30.4618 98.8065 32.9391 101.784 33.1188V33.1406H115.124V20.3621C115.102 17.0028 112.422 14.3023 109.11 14.3023V14.2969ZM103.118 25.7523C103.215 25.7305 103.318 25.7305 103.394 25.7142H109.786V29.0735H103.415C103.318 29.0735 103.215 29.0517 103.14 29.0353C102.383 28.8774 101.811 28.1969 101.811 27.3965C101.811 26.5962 102.346 25.9156 103.124 25.7577L103.118 25.7523Z" />
					</svg>
				</Link>

				<div className="flex items-center">
					<div className="border-r pr-4 mr-4 flex items-center gap-2 text-sm">
						<div className="h-6 w-6 rounded-full flex items-center justify-center bg-primary text-white">
							<svg
								className="w-3 h-3"
								viewBox="0 0 24 24"
								fill="currentColor"
							>
								<path
									fillRule="evenodd"
									d="M19.5 9.75a.75.75 0 01-.75.75h-4.5a.75.75 0 01-.75-.75v-4.5a.75.75 0 011.5 0v2.69l4.72-4.72a.75.75 0 111.06 1.06L16.06 9h2.69a.75.75 0 01.75.75z"
									clipRule="evenodd"
								/>
								<path
									fillRule="evenodd"
									d="M1.5 4.5a3 3 0 013-3h1.372c.86 0 1.61.586 1.819 1.42l1.105 4.423a1.875 1.875 0 01-.694 1.955l-1.293.97c-.135.101-.164.249-.126.352a11.285 11.285 0 006.697 6.697c.103.038.25.009.352-.126l.97-1.293a1.875 1.875 0 011.955-.694l4.423 1.105c.834.209 1.42.959 1.42 1.82V19.5a3 3 0 01-3 3h-2.25C8.552 22.5 1.5 15.448 1.5 6.75V4.5z"
									clipRule="evenodd"
								/>
							</svg>
						</div>
						(800) 983-4289
					</div>

					<button
						onClick={() => setOpenMenu(!openMenu)}
						className="z-10 flex items-center justify-center border rounded-full w-9 h-9 focus:outline-none"
					>
						{!openMenu ? (
							<svg
								className="w-5 h-5"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
								xmlns="http://www.w3.org/2000/svg"
							>
								<path
									strokeLinecap="round"
									strokeLinejoin="round"
									strokeWidth="2"
									d="M4 6h16M4 12h16m-7 6h7"
								></path>
							</svg>
						) : (
							<svg
								className="w-5 h-5"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
								xmlns="http://www.w3.org/2000/svg"
							>
								<path
									strokeLinecap="round"
									strokeLinejoin="round"
									strokeWidth="2"
									d="M6 18L18 6M6 6l12 12"
								></path>
							</svg>
						)}
					</button>
				</div>
			</div>

			<div
				id="mainNavigationMenuItems"
				className={`${
					!openMenu && "pointer-events-none"
				} md:pointer-events-auto section-wrapper pl-0 pr-0 md:pl-8 md:pr-8 fixed top-0 bottom-0 left-0 right-0 flex flex-col md:flex-row items-centers md:justify-between md:relative md:bg-transparent`}
			>
				<div className="h-16 relative items-center hidden md:flex">
					<a
						id="mainSiteLogo"
						href="/"
						className="pt-0.5 pb-3 transition-transform duration-300 origin-top-left sscale-125"
					>
						<svg
							id="logoSvg"
							className="h-7"
							viewBox="0 0 116 34"
							fill="currentColor"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path
								d="M45.6952 12.9354C49.0073 12.9354 51.6872 10.2295 51.6872 6.89741C51.6872 3.56533 49.0019 0.859375 45.6952 0.859375C42.3885 0.859375 39.7031 3.56533 39.7031 6.89741C39.7031 10.2295 42.3885 12.9354 45.6952 12.9354Z"
								fill="#F8B400"
							/>
							<path d="M11.3357 14.2969H0V33.1133H5.33827V18.3367H10.0011C11.1142 18.3367 12.0057 19.2351 12.0057 20.3567V33.1351H17.344V20.3567C17.344 16.9974 14.6424 14.2969 11.3303 14.2969H11.3357Z" />
							<path d="M70.0436 29.0717H66.5478C65.5158 28.99 64.7 28.1297 64.7 27.0517V18.3132H70.0382V14.2733H64.7V7.55469H59.3617V14.2733H57.0222L54.0234 18.3132H59.3617V27.068C59.3617 30.4056 62.0416 33.1061 65.3537 33.1061H70.0166V30.8684L70.0382 29.0662L70.0436 29.0717Z" />
							<path d="M55.7192 14.2969H49.7056L45.7181 19.6761L41.709 14.2969H35.6953L42.7031 23.6942L35.6953 33.1133H41.709L45.7181 27.7341L49.7056 33.1133H55.7192L48.7114 23.6942L55.7192 14.2969Z" />
							<path d="M92.7187 14.2969L88.0558 26.2749L83.3929 14.2969H78.0547L85.3975 33.1133H90.7196L98.0624 14.2969H92.7241H92.7187Z" />
							<path d="M76.7211 14.2969H71.3828V33.1133H76.7211V14.2969Z" />
							<path d="M76.7211 7.57812H71.3828V12.9574H76.7211V7.57812Z" />
							<path d="M28.0114 29.0718C26.3257 29.0718 24.8938 27.6725 24.2995 25.7125H37.1427C37.2832 25.07 37.3588 24.3949 37.3588 23.6926C37.3588 18.493 33.1714 14.2734 28.0114 14.2734C22.8515 14.2734 18.6641 18.493 18.6641 23.6926C18.6641 28.8921 22.8515 33.1117 28.0114 33.1117H34.3439L37.3426 29.0718H28.0114ZM28.0114 18.3351C29.6972 18.3351 31.129 19.7343 31.7234 21.6944H24.2995C24.8938 19.7126 26.3419 18.3351 28.0114 18.3351Z" />
							<path d="M109.11 14.2969H99.3846L97.8177 18.3367H107.781C108.894 18.3367 109.786 19.2351 109.786 20.3567V21.6961H101.784V21.7178C98.8065 21.8975 96.4453 24.3748 96.4453 27.4183C96.4453 30.4618 98.8065 32.9391 101.784 33.1188V33.1406H115.124V20.3621C115.102 17.0028 112.422 14.3023 109.11 14.3023V14.2969ZM103.118 25.7523C103.215 25.7305 103.318 25.7305 103.394 25.7142H109.786V29.0735H103.415C103.318 29.0735 103.215 29.0517 103.14 29.0353C102.383 28.8774 101.811 28.1969 101.811 27.3965C101.811 26.5962 102.346 25.9156 103.124 25.7577L103.118 25.7523Z" />
						</svg>
					</a>
				</div>

				<nav
					role="off-canvas"
					className={`${
						!openMenu && "opacity-0 pointer-events-none"
					} md:opacity-100 md:pointer-events-auto fixed md:static inset-0 pt-8 md:pt-0 px-4 md:px-0 bg-canvas w-full flex items-start md:items-center justify-center mt-14 md:w-auto md:mt-0`}
				>
					<ul className="w-full relative z-50 shidden md:flex flex-col justify-center items-center md:h-full md:flex-row md:gap-x-8">
						{/* <li className="w-full h-fulls text-center md:w-auto md:text-left">
                            <a
                                href="https://www.nictanzania.co.tz"
                                className="flex items-center h-full px-1 py-4 md:py-0 md:mr-4 text-3xl md:text-base text-primary font-bold text-center"
                            >
                                Home
                            </a>
                        </li> */}
						{links.map((link, index) => {
							// const selected = page.url.indexOf(link.url) == 0;
							const selected = false;

							return (
								<li
									key={index}
									className="w-full h-full text-center md:w-auto md:text-left"
								>
									<Link
										href={link.url}
										className={`${
											selected
												? "text-primary border-primary"
												: "border-transparent text-content/50 md:hover:text-content/80"
										} md:border-b-2 flex items-center justify-center gap-1 h-full px-2 py-4 md:py-0 text-2xl md:text-sm font-semibold text-center`}
										onClick={() => setOpenMenu(false)}
									>
										{link.label} {selected}
										{link.label != links.at(-1).label && (
											<svg
												className="mt-0.5 w-3 h-3"
												fill="none"
												viewBox="0 0 24 24"
												strokeWidth={1.5}
												stroke="currentColor"
											>
												<path
													strokeLinecap="round"
													strokeLinejoin="round"
													d="M19.5 8.25l-7.5 7.5-7.5-7.5"
												/>
											</svg>
										)}
									</Link>
								</li>
							);
						})}

						<div className="border-l pl-6 flex items-center gap-2 text-sm">
							<div className="h-6 w-6 rounded-full flex items-center justify-center bg-primary text-white">
								<svg
									className="w-3 h-3"
									viewBox="0 0 24 24"
									fill="currentColor"
								>
									<path
										fillRule="evenodd"
										d="M19.5 9.75a.75.75 0 01-.75.75h-4.5a.75.75 0 01-.75-.75v-4.5a.75.75 0 011.5 0v2.69l4.72-4.72a.75.75 0 111.06 1.06L16.06 9h2.69a.75.75 0 01.75.75z"
										clipRule="evenodd"
									/>
									<path
										fillRule="evenodd"
										d="M1.5 4.5a3 3 0 013-3h1.372c.86 0 1.61.586 1.819 1.42l1.105 4.423a1.875 1.875 0 01-.694 1.955l-1.293.97c-.135.101-.164.249-.126.352a11.285 11.285 0 006.697 6.697c.103.038.25.009.352-.126l.97-1.293a1.875 1.875 0 011.955-.694l4.423 1.105c.834.209 1.42.959 1.42 1.82V19.5a3 3 0 01-3 3h-2.25C8.552 22.5 1.5 15.448 1.5 6.75V4.5z"
										clipRule="evenodd"
									/>
								</svg>
							</div>
							(800) 983-4289
						</div>

						<div className="border-t mt-4 pt-4 flex md:hidden flex-col gap-3">
							<a
								href="#"
								className="hover:opacity-90 rounded-full h-12 px-4 text-md font-semibold  group min-w-[80px] flex items-center justify-center text-center focus:outline-none"
							>
								Login
							</a>
							<a
								href="#"
								className="bg-primary text-white shadow hover:opacity-90 rounded-full h-12 px-4 text-md font-semibold  group min-w-[80px] flex items-center justify-center text-center focus:outline-none"
							>
								Start for free
							</a>
						</div>
					</ul>
				</nav>

				<div className="hidden md:flex items-center gap-3">
					<div className="border-r pr-4 flex items-center gap-2 text-sm">
						<button>
							<svg
								className="h-6 w-6 opacity-60 hover:opacity-80"
								viewBox="0 0 32 32"
								fill="currentColor"
							>
								<path d="M16 3C13.4288 3 10.9154 3.76244 8.77759 5.1909C6.63975 6.61935 4.97351 8.64968 3.98957 11.0251C3.00563 13.4006 2.74819 16.0144 3.2498 18.5362C3.75141 21.0579 4.98953 23.3743 6.80762 25.1924C8.6257 27.0105 10.9421 28.2486 13.4638 28.7502C15.9856 29.2518 18.5995 28.9944 20.9749 28.0104C23.3503 27.0265 25.3807 25.3603 26.8091 23.2224C28.2376 21.0846 29 18.5712 29 16C28.9964 12.5533 27.6256 9.24882 25.1884 6.81163C22.7512 4.37445 19.4467 3.00364 16 3ZM25.7963 11H21.3675C20.8572 8.99189 20.0001 7.0883 18.835 5.375C20.3236 5.77503 21.7119 6.48215 22.9108 7.45091C24.1097 8.41967 25.0926 9.62861 25.7963 11ZM16 5.01375C17.5 6.63875 18.625 8.7075 19.2963 11H12.7038C13.375 8.7075 14.5 6.64125 16 5.01375ZM12 16C12.0002 14.9948 12.0838 13.9914 12.25 13H19.75C20.0817 14.9862 20.0817 17.0138 19.75 19H12.25C12.0838 18.0086 12.0002 17.0052 12 16ZM12.7038 21H19.2963C18.625 23.2925 17.5 25.3587 16 26.9862C14.5 25.3587 13.375 23.2925 12.7038 21ZM18.835 26.625C20.0001 24.9117 20.8572 23.0081 21.3675 21H25.7963C25.0926 22.3714 24.1097 23.5803 22.9108 24.5491C21.7119 25.5178 20.3236 26.225 18.835 26.625ZM21.7763 19C22.0746 17.0111 22.0746 14.9889 21.7763 13H26.5838C27.1388 14.9615 27.1388 17.0385 26.5838 19H21.7763Z" />
							</svg>
						</button>
					</div>

					<a
						href="#"
						className="opacity-80 hover:opacity-90 rounded-full h-9 px-3 text-sm font-semibold  group min-w-[80px] flex items-center justify-center text-center focus:outline-none"
					>
						Login
					</a>
					<a
						href="#"
						className="bg-primary text-white shadow hover:opacity-90 rounded-full h-9 px-4 text-sm font-semibold  group min-w-[80px] flex items-center justify-center text-center focus:outline-none"
					>
						Start for free
					</a>
				</div>
			</div>
		</section>
	);
}
